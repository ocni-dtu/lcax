...
fn apply_rules<'a>(mut validator: Validator<'a, Message>, schema: &'a ValidationSchema) -> Validator<'a, Message> {
    for (key, field) in schema.fields() {
        ...
        if let Some(Some(rule)) = field.get::<Option<ValidationRules>>() {
            validator = match_rule(&rule.clone(), converted_key, validator)
...
fn match_rule<'a>(
    ...
    match rule {
        ValidationRules::Range(range) => {
            validator = validator.rule(key.as_str(), Range::new(range.min..range.max));
        }
        ValidationRules::Required => {
            validator = validator.rule(key.as_str(), Required);
        }
        ValidationRules::Equal(equal) => {
            validator = validator.rule(key.as_str(), rules::Equal(equal.clone()));
        }
        ValidationRules::Includes(includes) => {
            validator = validator.rule(key.as_str(), rules::Includes(includes.clone()));
        }
    }
    validator
}
