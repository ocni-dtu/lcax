...
use crate::project::{
    Assembly, Classification, ImpactCategoryKey, LCAxProject, LifeCycleStage, Location, Product,
    SoftwareInfo,
};
        ...
        for _field in fields {
            ...
            if field.type_id() == TypeId::of::<String>() {
                match row.get_string(field_iter_count) {
                    ...
                    Err(_) => continue
                    //Err(_) => println!("Error getting string value for {_field}"),
                };
                ...
                match row.get_double(field_iter_count) {
                    ...
                    Err(_) => continue
                    //Err(_) => println!("Error getting f64 value for {_field}"),
                };
                ...
                match row.get_long(field_iter_count) {
                    ...
                    Err(_) => continue
                    //Err(_) => println!("Error getting i64 value for {_field}"),
                };
            }
            field_iter_count += 1;
        }
...
pub fn add_slice_element(project: &mut LCAxProject, element: &SLiCEElement) {
    // create uuid5 based on sha1 of element_class_sfb
    // if uuid doesn't exist in project.assemblies, then create a new assembly
    // and create a product with uuid of the product is based on sha1 of worksection_class_sfb + techflow_name_mmg
    // then add an impact data
    // if uuid exists in assemblies
    // create an uuid of the product is based on sha1 of worksection_class_sfb + techflow_name_mmg
    // if the product uuid exists, then add data to its impact data
    // if the product uuid doesn't exist, then create a new product and add data to its impact data
    ...
    if !project.assemblies.contains_key(&assembly_uuid) {
        let mut assembly = Assembly::from((assembly_uuid.as_str(), element));
        let product = Product::from((product_uuid.as_str(), element));
        ...
        if !assembly.products.contains_key(&product_uuid) {
            ...
            let mut product = assembly.products.get_mut(&product_uuid).unwrap();
            add_impact_data(&mut product, element);
        }
    }
}
...
impl From<(&str, &SLiCEElement)> for Assembly {
    fn from(element: (&str, &SLiCEElement)) -> Self {
        let (uid, element) = element;
        Assembly {
            id: uid.to_string(),
...
impl From<(&str, &SLiCEElement)> for Product {
    fn from(element: (&str, &SLiCEElement)) -> Self {
        let (uid, element) = element;
...
fn add_impact_data(product: &mut Product, element: &SLiCEElement) {}
